...
local function get_overseer_task_status(status)
  ...
  if not package.loaded.overseer then return result end

  local tasks = require('overseer.task_list').list_tasks { unique = true }
  local tasks_by_status = require('overseer.util').tbl_group_by(tasks, 'status')
  if not tasks_by_status[status] then return result end
...
return {
  ...
  config = function()
    ...
    if vim.g.colors_name == 'catppuccin-mocha' then override_hl() end
